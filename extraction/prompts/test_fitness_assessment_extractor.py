#!/usr/bin/env python3
"""Fitness Assessment extractor using Claude API with text extraction."""
...
# The Fitness Assessment extraction prompt
FITNESS_ASSESSMENT_PROMPT = """**Parse this medical laboratory report and convert it to standardized JSON format. Extract all available Fitness Assessment biomarkers. Return only a single JSON object with patient information and biomarker values (skip all null/missing values).**
...
3. **Handle different value types**:
   ...
   - Interpretation: Extract the complete interpretation/assessment text, but stop before any "Limitation:" sections. Include the full descriptive assessment text, whatever it may be, but exclude any limitation content that starts with "Limitation:".
...
**IMPORTANT**: Return ONLY a valid JSON object with this structure:
{
  ...
  "sex": "Male" or "Female",
  "cycle": "All",
  "BALANCE_AND_POSTURE": value,
  "UPPER_BODY_STRENGTH": value,
  "LOWER_BODY_STRENGTH": value,
  "AGILITY": value,
  "POWER": value,
  "VO2_MAX": value,
  "UPPER_BODY_FLEXIBILITY": value,
  "LOWER_BODY_FLEXIBILITY": value,
  "HAND_EYE_COORDINATION": value,
  "REACTION_TIME": value,
  "TOTAL_FITNESS_SCORE": value,
...
def process_pdf_with_claude(pdf_path: str, api_key: str) -> dict | None:
    """Process PDF and extract Fitness Assessment data."""
    try:
        ...
        message = client.messages.create(
            ...
            messages=[
                {
                    "role": "user",
                    "content": f"{FITNESS_ASSESSMENT_PROMPT}\n\n---REPORT TEXT---\n{text}"
                }
            ]
        )
...
def main():
    parser = argparse.ArgumentParser(description="Extract Fitness Assessment data from PDF")
    ...
    if result:
        ...
        if biomarkers:
            print(f"\nExtracted {len(biomarkers)} Fitness Assessment biomarkers:")
