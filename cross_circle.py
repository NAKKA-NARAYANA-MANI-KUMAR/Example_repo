...
class CustomArcFlowable(Flowable):
    ...
    def draw_pointer_circle(self, drawing, px, py, p2x, p2y, result_color, radius=3):
        dx = px - p2x
        dy = py - p2y
        ...
        cx = px + dx * radius
        cy = py + dy * radius
        circle = Circle(cx, cy, radius, fillColor=HexColor("#FFFFFF"), strokeColor=result_color, strokeWidth=1.02784)
        drawing.add(circle)
    ...
    def draw(self):
        ...
        exact_segments = (value / 100) * total_segments
        ...
        if value > 0:
            if full_segments == total_segments and remainder == 0:
                x1, y1, x2, y2, x3, y3 = segments[-1]
                end_x, end_y = x3, y3
                p2x, p2y = x2, y2
            elif 'p3_split' in locals():
                end_x, end_y = p3_split
                p2x, p2y = p2_split
            else:
                end_x, end_y = segments[0]
                p2x, p2y = segments[1][0], segments[1][1]

            self.draw_pointer_circle(drawing, end_x, end_y, p2x, p2y, result_color)
...
class CustomArcFlowable(Flowable):
    ...
    def draw_pointer_circle(self, drawing, px, py, p2x, p2y, result_color, radius=3):
        """Draws the pointer circle at the end of the foreground arc."""
        dx = px - p2x
        dy = py - p2y
        length = (dx ** 2 + dy ** 2) ** 0.5
        if length != 0:
...
class RowFlowable(Flowable):
    ...
    def _get_chart_height(self, data):
        if data.get("type") == "pie_chart":
            ...
            PIE_SIZE = 60
            legend_height = len(labels) * INDICATOR_SIZE + (len(labels) - 1) * SPACE_BETWEEN_INDICATORS
            return max(legend_height, PIE_SIZE) + 20
        else:
            return 54
...
class DomainHeaderFlowable(Flowable):
    ...
    def __init__(self, header_text: str):
        super().__init__()
        self.header_text = header_text.upper()
