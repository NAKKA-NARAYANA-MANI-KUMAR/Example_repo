...
class RoundedBox(Flowable):
    def __init__(self, width=A4[0]-64, height=None, content=None, corner_radius=8,border_radius=0.4,stroke_color=PMX_GREEN,fill_color=colors.white ):
        super().__init__()
        self.width = width
        self.height = height  
...
class PrescriptionOnlyTemplate(PrescriptionOnlyPMXBasePage):
    ...
    def __init__(self):
        ...
        self.base_path = (
            Path(__file__).parent.parent.parent.parent.parent.parent / "staticfiles" / "icons"
        )
        ...
        for logo_path in possible_paths:
            try:
                if os.path.exists(logo_path):
                    ...
                    if svg is not None:
                        return PrescriptionOnlySVGImage(logo_path, width=width, height=height)
...
class PrescriptionPage(PrescriptionOnlyTemplate):
    ...
    def _create_prescription_table(self, medications: list) -> Table:
        ...
        for i, med in enumerate(medications, 1):
            ...
            strength = med.get("strength", "")
            form=med.get("dosage","").split(" ")[1].lower()
            if form=="tablets" or form=="tablet":
                form="tablet"
            elif form=="capsules" or form=="capsule":
                form="capsule"
            elif form=="ml":
                form="syrup"
            elif form=="tsp" or form=="tbsp":
                form="powder"
    ...
    def create_diagnosis_pill_table(self, diagnoses: list,title) -> Table:
        bullets = []
        icon_path = str(self.base_path / "bullet_text_icon.svg")
        ...
        for i, item in enumerate(diagnoses):
            bullets.append([Paragraph(f"• {item.get('name','Unknown')}", self.styles["bullet_after_text"])])
        ...
        if bullets:
            ...
            return RoundedBox(content=bullet_table, corner_radius=16,border_radius=0.1,stroke_color=PMX_GREEN,fill_color=colors.white)
        return None
    ...
    def add_top_left_image(self, x,y, width=34.7, height=80) -> list:
        """
        ...
        """
        image_path=str(self.base_path / "x_logo.png")
    ...
    def _add_logo_to_bottom_left(self, canvas, doc):
        ...
        if isinstance(logo, PrescriptionOnlySVGImage):
            drawing = svg2rlg(logo.filename)  # works because filename is now defined
