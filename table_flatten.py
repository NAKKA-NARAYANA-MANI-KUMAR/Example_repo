...
class PrescriptionPage(PrescriptionOnlyTemplate):
    ...
    def _build_styled_table(self, table_data, col_widths, top_rounded=True, bottom_rounded=True) -> Table:
        """Build a styled table with optional top/bottom rounded corners."""
        ...
        # Add rounded corners
        if top_rounded and bottom_rounded:
            style.append(("ROUNDEDCORNERS", [20, 20, 20, 20]))
        elif top_rounded:
            style.append(("ROUNDEDCORNERS", [20, 20, 0, 0]))
        elif bottom_rounded:
            style.append(("ROUNDEDCORNERS", [0, 0, 20, 20]))
        else:
            style.append(("ROUNDEDCORNERS", [0, 0, 0, 0]))
    ...
    def build_prescription_tables_with_rounded_corners(self, full_table_data, col_widths, rows_per_page=7):
        """
        Split full table into multiple tables per page, and apply top/bottom corner styles.
        """
        ...
        for i in range(0, total_rows, rows_per_page):
            ...
            top_rounded = (i == 0)
            bottom_rounded = (i + rows_per_page >= total_rows)

            table = self._build_styled_table(chunk_data, col_widths, top_rounded=top_rounded, bottom_rounded=bottom_rounded)
            tables.append(table)
    ...
    def _create_prescription_table(self, medications: list) -> Table:
        ...
        return self.build_prescription_tables_with_rounded_corners(table_data, col_widths)
    ...
    def generate(self, data: dict) -> list:
        ...
        if medications:
            for table in self._create_prescription_table(medications):
                story.append(table)
                story.append(Spacer(1, self.PAGE_MARGIN / 4))
