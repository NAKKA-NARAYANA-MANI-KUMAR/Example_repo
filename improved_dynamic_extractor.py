...
def extract_medications_improved(text):
    """Improved medication extraction with better parsing"""
    medications = []
    ...
    lines = [line.strip() for line in prescription_section.split('\n') if line.strip()]
    ...
    while i < len(lines):
        ...
        # Check if line starts with a number followed by medication name
        if re.match(r'^\d+\s+[A-Za-z]', line):
            med_data = {}
            
            # Extract medication number and name
            match = re.match(r'^(\d+)\s+(.+)', line)
            if match:
                med_data["med_number"] = match.group(1)
                med_data["name"] = match.group(2).strip()
            ...
            while j < len(lines) and j < i + 10:  # Look ahead max 10 lines
                ...
                # Check if next line starts with a number (new medication)
                if re.match(r'^\d+\s+[A-Za-z]', next_line):
                    break
                
                # Extract dosage information
                if any(keyword in next_line.lower() for keyword in ['tablet', 'capsule', 'mg', 'mcg', 'cream', 'ml']):
                    # Extract dosage
                    dosage_match = re.search(r'(\d+)\s*(tablet|capsule|mg|mcg|cream|ml)', next_line, re.IGNORECASE)
                    if dosage_match:
                        med_data["dosage"] = f"{dosage_match.group(1)} {dosage_match.group(2)}"
                    
                    # Extract strength
                    strength_match = re.search(r'(\d+[mgmcg]*)', next_line, re.IGNORECASE)
                    if strength_match and ('mg' in next_line.lower() or 'mcg' in next_line.lower()):
                        med_data["strength"] = strength_match.group(1)
                
                # Extract frequency (pattern like 0-0-1, 1-0-0, etc.)
                elif re.match(r'^[0-9-]+$', next_line) and len(next_line) <= 10:
                    med_data["frequency"] = next_line
                
                # Extract timing
                elif any(keyword in next_line.lower() for keyword in ['after', 'before', 'morning', 'evening', 'lunch', 'dinner', 'breakfast']):
                    med_data["timing"] = next_line
                    med_data["instructions"] = next_line
                
                # Extract duration
                elif 'month' in next_line.lower():
                    duration_match = re.search(r'(\d+\s*months?)', next_line, re.IGNORECASE)
                    if duration_match:
                        med_data["duration"] = duration_match.group(1)
                
                # Extract start day
                elif 'day' in next_line.lower():
                    day_match = re.search(r'(Day\s*\d+)', next_line, re.IGNORECASE)
                    if day_match:
                        med_data["start_from"] = day_match.group(1)
                
                # Extract availability
                elif any(keyword in next_line.lower() for keyword in ['available', 'pmx', 'link', 'buy']):
                    med_data["availability"] = next_line
                    med_data["available_in_clinic"] = "pmx" in next_line.lower() or "available" in next_line.lower()
            ...
            if med_data.get("name"):
                cleaned_med = {
                    ...
                    "dosage": med_data.get("dosage", "1 tablet"),
                    "frequency": med_data.get("frequency", "1-0-0"),
                    "duration": med_data.get("duration", "2 months"),
                    "instructions": med_data.get("instructions", "After meal"),
                    "active_ingredients": "",
                    "start_from": med_data.get("start_from", "Day 1"),
                    "timing": med_data.get("timing", "After meal"),
                    "available_in_clinic": med_data.get("available_in_clinic", False),
                    "external_url": ""
                }
...
def main():
    ...
    medications = extract_medications_improved(text)
    ...
    # Save to JSON file
    output_file = "improved_dynamic_extracted_report_data.json"
