#!/usr/bin/env python3
"""Extract Thyrocare data using hybrid approach - optimized sequential processing."""
...
# Full Thyrocare prompt for hybrid processing
HYBRID_PROMPT = """**Parse this medical laboratory report and convert it to standardized JSON format. Extract all available biomarkers and convert them to the specified units below. Return only a single JSON object with patient information and biomarker values (skip all null/missing values).**
...
# AMYLASE:AMYL : U/L
# EPSTEIN BARR VIRAL CAPSID ANTIGEN - IGG : EBVCG : NTU
...
# Anti-Thyroid Peroxidase:ANTI_TPO : IU/mL
...
# HDL CHOLESTEROL - DIRECT:HCHO : mg/dL
...
# HOMA INSULIN RESISTANCE INDEX:HOMIR : Index
...
EPITHELIAL CELLS:UEPIT : cells/HPF
EPSTEIN BARR VIRAL CAPSID ANTIGEN - IGG:EBVCG : NTU
EST. GLOMERULAR FILTRATION RATE (eGFR):EGFR : mL/min/1.73 m2
ESTRADIOL/OESTROGEN (E2):E2 : pg/mL
FASTING BLOOD SUGAR(GLUCOSE):FBS : mg/dl
FERRITIN:FERR : ng/mL
FOLATE:FOLI : ng/mL
FOLLICLE STIMULATING HORMONE (FSH):FSH : mIU/L
FREE TESTOSTERONE:FTES : pg/mL
FREE THYROXINE (FT4):FT4 : ng/dl
FREE TRIIODOTHYRONINE (FT3):FT3 : pg/mL
GAMMA GLUTAMYL TRANSFERASE (GGT):GGT : U/L
HDL / LDL RATIO:HD/LD : Ratio
HDL CHOLESTEROL - DIRECT:HCHO : mg/dL
HEMATOCRIT (PCV):PCV : %
HEMOGLOBIN:HB : g/dl
HEPATITIS B SURFACE ANTIGEN(HBSAG) RAPID TEST:HBSAG : #
HIGH SENSITIVITY C-REACTIVE PROTEIN (HS-CRP):HSCRP : mg/L
HOMA INSULIN RESISTANCE INDEX:HOMIR : Index
HOMOCYSTEINE:HOMO : µmol/L
HbA1c:HBA : %
IMMATURE GRANULOCYTE PERCENTAGE (IG%):IG% : %
IMMATURE GRANULOCYTES (IG):IG : X 10³ / µL
INSULIN - FASTING:INSFA : μU/ml
INSULIN LIKE GROWTH FACTOR 1:INGF1 : ng/mL
IRON:IRON : µg/dl
LACTATE DEHYDROGENASE (LDH):LDH : U/L
LDL / HDL RATIO:LDL/ : Ratio
LDL CHOLESTEROL - DIRECT:LDL : mg/dL
LEUCOCYTE ESTERASE:ULEST : #
LIPASE:LASE : U/L
LIPOPROTEIN (a) [Lp(a)]:LPA : mg/dl
LUTEINISING HORMONE (LH):LH : mIU/L
LYMPHOCYTES - ABSOLUTE COUNT:ALYM : X 10³ / µL
LYMPHOCYTES PERCENTAGE:LYMPH : %
MAGNESIUM:MG : mg/dL
MEAN CORP.HEMO. CONC (MCHC):MCHC : g/dL
MEAN CORPUSCULAR HEMOGLOBIN (MCH):MCH : pg
MEAN CORPUSCULAR VOLUME (MCV):MCV : fL
MEAN PLATELET VOLUME (MPV):MPV : fL
MONOCYTES - ABSOLUTE COUNT:AMON : X 10³ / µL
...
POTASSIUM:POT : mmol/L
PROGESTERONE:PROG : ng/mL
PROLACTIN (PRL):PRL : ng/ml
PROSTATE SPECIFIC ANTIGEN (PSA):PSA : ng/mL
PROTEIN - TOTAL:PROT : gm/dL
QUANTITATIVE INSULIN SENSITIVITY INDEX:QUICKI : Index
RED BLOOD CELLS:URBC : cells/HPF
RED CELL DISTRIBUTION WIDTH (RDW - CV):RDCV : %
RED CELL DISTRIBUTION WIDTH - SD (RDW-SD):RDWSD : fL
RHEUMATOID FACTOR (RF):RFAC : ng/mL
SERUM ALB/GLOBULIN RATIO:A/GR : Ratio
SERUM GLOBULIN:SEGB : gm/dL
SERUM ZINC:SEZN : μg/dL
SEX HORMONE BINDING GLOBULIN (SHBG):SHBG : nmol/L
SGOT / SGPT RATIO:OT/PT : Ratio
SODIUM:SOD : mmol/L
SPECIFIC GRAVITY:SPGR : #
TC/ HDL CHOLESTEROL RATIO:TC/H : Ratio
TESTOSTERONE:TEST : ng/dl
TOTAL CHOLESTEROL:CHOL : mg/dl
TOTAL IRON BINDING CAPACITY:TIBC : µg/dL
TOTAL LEUCOCYTES COUNT (WBC):LEUC : X 10^3/μL
TOTAL RBC:RBC : X 10^6/μL
TOTAL_CHOLESTEROL:TOTAL_CHOLESTEROL : mg/dL
TRANSFERRIN SATURATION %:TRANSAT : %
TRIG / HDL RATIO:TRI/H : Ratio
TRIGLYCERIDES:TRIG : mg/dL
TSH - ULTRASENSITIVE:USTSH : μIU/mL
UREA (CALCULATED):UREAC : mg/dL
UREA / SR.CREATININE RATIO:UR/CR : Ratio
URI. ALBUMIN/CREATININE RATIO (UA/C):UA/C : µg/mg of Creatinine
URIC ACID:URIC : mg/dL
URINARY BILIRUBIN:UBIL : #
URINARY GLUCOSE:UGLU : #
URINARY LEUCOCYTES (PUS CELLS):ULEUC : cells/HPF
URINARY MICROALBUMIN:UALB : μg/mL
URINARY PROTEIN:UPROT : #
URINE BLOOD:UBLD : #
URINE KETONE:UKET : #
UROBILINOGEN:UBNGN : #
VITAMIN B-12:VITB : pg/mL
...
def split_text_into_chunks(text: str, chunk_size: int = 40000) -> list[str]:
    """Split text into larger chunks with better overlap."""
    chunks = []
    start = 0
    ...
    while start < len(text):
        ...
        chunks.append(chunk)
        start = end - 3000  # Larger overlap for better coverage
        
    return chunks

def process_chunk_with_claude(chunk: str, api_key: str, chunk_num: int, total_chunks: int, max_retries: int = 3) -> dict | None:
    """Process a single chunk with Claude API with retry logic."""
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                wait_time = 15  # Wait 15 seconds before retry
...
def process_pdf_hybrid(pdf_path: str, api_key: str) -> dict | None:
    """Process PDF using hybrid approach - optimized sequential processing."""
    ...
    chunks = split_text_into_chunks(full_text, 40000)
...
def main():
    parser = argparse.ArgumentParser(description="Extract Thyrocare data using hybrid approach.")
    ...
    output_path = args.output or str(Path(args.pdf_path).with_suffix('')) + '_hybrid.json'
    ...
    result = process_pdf_hybrid(args.pdf_path, api_key)
    end_time = time.time()
